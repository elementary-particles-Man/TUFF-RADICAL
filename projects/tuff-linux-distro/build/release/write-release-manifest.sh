...
if ! command -v cargo >/dev/null 2>&1; then
    echo "[ERROR] cargo is required to generate the release manifest." >&2
    exit 1
fi
exec env CARGO_TARGET_DIR="${TOOLS_TARGET_DIR}" cargo run --quiet --manifest-path "${TOOLS_MANIFEST}" -- write-release-manifest "$@"
