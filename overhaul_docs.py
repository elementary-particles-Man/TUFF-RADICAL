...
REPLACEMENTS = {
    ...
    r"bzImage": "tuff-radical-kernel.efi (Sovereign Core)",
    r"Linuxベース": "Pure Rust ベアメタルベース",
    r"LinuxOS": "TUFF-RADICAL-KERNEL",
    r"ハイパーバイザ": "Sovereign Executive (非同期エグゼキュータ)",
...
SPECIFIC_REPLACEMENTS = [
    (r"上位OS（Windows / Linux / macOS等）の下位レイヤーで稼働するセキュリティ基盤OSです。", 
     "UEFIから主権を奪取し、完全なPure Rustベアメタル環境で稼働するセキュリティ基盤OS（Sovereign Core）です。"),
...
def process_file(filepath):
    ...
    if "## 1. アーキテクチャ概要" in content and "Sovereign ZRAM" not in content:
        content = content.replace("## 1. アーキテクチャ概要", "## 1. アーキテクチャ概要\n" + ZRAM_PARAGRAPH_JP)
        
    if "## 1. Architecture Overview" in content and "Sovereign ZRAM" not in content:
        content = content.replace("## 1. Architecture Overview", "## 1. Architecture Overview\n" + ZRAM_PARAGRAPH_EN)

    # Inject into READMEs
    if os.path.basename(filepath) == "README_JP.md" and "Sovereign ZRAM" not in content:
        content = content.replace("## TUFF-RADICAL について", "## TUFF-RADICAL について\n" + ZRAM_PARAGRAPH_JP)
        
    if os.path.basename(filepath) == "README.md" and "Sovereign ZRAM" not in content:
